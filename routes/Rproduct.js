...
// 상품 등록 버튼 클릭시
router.post('/write', authenticate(adminOrUser), postUpProductImage('create'), controller.postProduct);

// 특정 상품 수정 페이지
router.get('/update', authenticate(adminOrUser), controller.getProductUpdate);

// 특정 상품 수정 버튼 클릭시
router.post('/update', authenticate(adminOrUser), postUpProductImage('update'), controller.postProductUpdate);
