...
exports.postSearch = async (req, res) => {
  try {
    ...
    if (result.length) {
      ...
      res.send({
        message: '해당 키워드에 맞는 중고리스트가 존재하지 않습니다.',
      });
    }
  } catch (err) {
...
exports.getProductList = async (req, res) => {
  try {
    ...
    // 이미지 불러오기
    const getImages = awaituploadImgProduct.getProductImg(
      req,
      productId,
      'product'
    );
    ...
    res
      .status(500)
      .json({ message: 'getProductList 서버 오류', err: err.message });
  }
};
...
exports.getProduct = async (req, res) => {
  try {
    ...
    // 이미지 불러오기
    const getImages = await uploadImgProduct.getProductImg(
      req,
      productId,
      'product'
    );
...
exports.getProductWrite = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'getProductWrite 서버 오류', err: err.message });
  }
};
...
exports.postProduct = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'postProduct 서버 오류', err: err.message });
  }
};
...
exports.getProductUpdate = async (req, res) => {
  try {
    ...
    if (!writer) {
      res
        .status(400)
        .json({ message: '로그인 유저와 작성자가 일치하지 않습니다.' });
    } else {
    ...
    res
      .status(500)
      .json({ message: 'getProductUpdate 서버 오류', err: err.message });
  }
};
...
exports.patchProductUpdate = async (req, res) => {
  try {
    ...
    if (!writer) {
      ...
      var imgFileArr = req.files;
      // 상품 이미지 s3에 삭제
      const findImg = await uploadImgProduct.deleteProductImg(
        req,
        productId,
        'product'
      );
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'deleteProduct 서버 오류', err: err.message });
  }
};
...
exports.postCategory = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'postCategory 서버 오류', err: err.message });
  }
};
